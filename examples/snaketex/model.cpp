...
void Model::render(int numTriangles) const {
  ...
  // aqui ele itera sobre a lista de coordenadas da cobra, faz a transformação 
  for(auto const pos : m_snakes_positions) {
    glm::mat4 m_viewMatrix{1.0f};
    m_viewMatrix = glm::lookAt(glm::vec3(0.0f, 7.5f, 7.5f),
                  glm::vec3(0.0f, 2.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));

    glm::mat4 model{1.0f};
    model = glm::translate(model, glm::vec3((-8.0f+(pos.x)), 1.0f, (-8.0f+(pos.y))));
    ...
    glm::mat4 model{0.1f};
    model = glm::translate(model, glm::vec3((-8.0f+food_loc.x), 1.0f, (-8.0f+food_loc.y)));
...
//void Model::renderFood(int numTriangles) const {
  ...
  //   //Aqui ele vai renderizar a comida, que é um cubo rotacionado e 75% menor na posição inicial da comida.
...
void Model::onUpdate(GameData m_gameData) {
checkFoodHasEaten();
  if (m_gameData.m_state == State::Playing && m_stepTimer.elapsed() > 0.5) {
    m_stepTimer.restart();
...
void Model::checkFoodHasEaten(){
  for (auto const &unit :  m_snakes_positions) {
    if (unit.x == (food_loc.x) && unit.y == (food_loc.y)) {
        std::uniform_int_distribution randomY(7, 13);
        std::uniform_int_distribution randomX(7, 13);

        food_loc = glm::vec2(randomX(m_randomEngine), randomY(m_randomEngine));
        increaseSize();
      }
   }
}
