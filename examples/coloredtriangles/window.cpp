...
void Window::onCreate() {
  ...
  auto const *vertexShader{R"gl(#version 410
  ...
  auto const *fragmentShader{R"gl(#version 410
...
void Window::onPaint() {
  ...
  std::uniform_real_distribution<float> rd_colors(0.0f, 1.0f);
  m_colors = {
      {{rd_colors(m_randomEngine), rd_colors(m_randomEngine), rd_colors(m_randomEngine), 1.0f},
       {rd_colors(m_randomEngine), rd_colors(m_randomEngine), rd_colors(m_randomEngine), 1.0f},
       {rd_colors(m_randomEngine), rd_colors(m_randomEngine), rd_colors(m_randomEngine), 1.0f}}};
  setupModel();
  ...
  abcg::glDrawArrays(GL_TRIANGLES, 0, 5);
